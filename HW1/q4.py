...
def standard_scale(xTrain, xTest):
    ...
    ##Used the sklearn preprocessing scale function to transform the data
...
def minmax_range(xTrain, xTest):
    ...
    ## Sklearn function to transform data to have min of 0 and max of 1
...
def add_irr_feature(xTrain, xTest):
    ...
    ##Create a random state and then generate irrelevant data with Gaussian distribution with 0 mean and std of 1
    rand = np.random.RandomState()

    xTrainDum = rand.normal(loc = 0.0, scale = 1.0, size = len(xTrain))

    xTrain['Irrev1'] = xTrainDum
    xTrain['Irrev2'] = xTrainDum

    xTestDum = rand.normal(loc = 0.0, scale = 1.0, size = len(xTest))

    xTest['Irrev1'] = xTestDum
    xTest['Irrev2'] = xTestDum
